...
c0_lines = [
    ...
    "        import shutil",
    "        # Use the primary account credentials & copy to writable dir in Kaggle",
    "        token_src = os.path.join(PROJECT_ROOT, 'token_1973.pickle')",
    "        creds_src = os.path.join(PROJECT_ROOT, 'credentials.json')",
    "        token_wt = '/kaggle/working/token_1973.pickle' if IS_KAGGLE else token_src",
    "        creds_wt = '/kaggle/working/credentials.json' if IS_KAGGLE else creds_src",
    "        if IS_KAGGLE:",
    "            if not os.path.exists(token_wt) and os.path.exists(token_src):",
    "                shutil.copy(token_src, token_wt)",
    "            if not os.path.exists(creds_wt) and os.path.exists(creds_src):",
    "                shutil.copy(creds_src, creds_wt)",
    "        ",
...
c7_lines = [
    ...
    "            token_src = os.path.join(PROJECT_ROOT, 'token_1973.pickle')",
    "            creds_src = os.path.join(PROJECT_ROOT, 'credentials.json')",
    "            token_wt = '/kaggle/working/token_1973.pickle' if os.path.exists('/kaggle/working') else token_src",
    "            creds_wt = '/kaggle/working/credentials.json' if os.path.exists('/kaggle/working') else creds_src",
    "            ",
    ...
    "                    # Standard naming convention requested by user",
    "                    model_name = f'detector_universal_{ts}.pt'",
    "                    dm.upload_file(weights_path, folder_yolo, remote_name=model_name, overwrite=True)",
    "                    logger.info(f'✅ Detector subido a yolo_model/{model_name}')",
    "                ",
    ...
    "                ",
    "                # 5. Subir Resultados Completos de YOLO (Matrices, Curvas F1, etc) en VIP ZIP",
    "                import zipfile",
    "                yolo_results_zip = f'/kaggle/working/yolo_metricas_completas_{SET_ID}.zip'",
    "                if os.path.exists(yolo_out):",
    "                    logger.info('📦 Comprimiendo métricas y gráficas completas de YOLO...')",
    "                    with zipfile.ZipFile(yolo_results_zip, \"w\", zipfile.ZIP_DEFLATED) as zipf:",
    "                        for root, dirs, files in os.walk(yolo_out):",
    "                            # Skip the weights folder because best.pt is already uploaded standalone",
    "                            if 'weights' in dirs: dirs.remove('weights')",
    "                            for file in files:",
    "                                file_path = os.path.join(root, file)",
    "                                arcname = os.path.relpath(file_path, yolo_out)",
    "                                zipf.write(file_path, arcname)",
    "                    # Subir a carpeta raiz de modelos en Drive",
    "                    dm.upload_file(yolo_results_zip, DRIVE_MODELS_FOLDER_ID, overwrite=True)",
    "                    logger.info('✅ Archivo complejo de métricas YOLO subido a Drive.')",
    "                ",
    ...
    "",
    "    # =================================================================",
    "    # IRON-CLAD FALLBACK: Universal ZIP Export for Kaggle UI",
    "    # =================================================================",
    "    import zipfile, datetime",
    "    ts = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')",
    "    model_name_zip = f'detector_universal_{ts}.pt'",
    "    zip_path = f'/kaggle/working/lego_models_{SET_ID}.zip'",
    "    logger.info(f'📦 Creando Backup ZIP Manual en: {zip_path}')",
    "    try:",
    "        with zipfile.ZipFile(zip_path, \"w\", zipfile.ZIP_DEFLATED) as zipf:",
    "            # Add YOLO Weights",
    "            weights_root = os.path.join(yolo_out, 'weights')",
    "            best_pt = os.path.join(weights_root, 'best.pt')",
    "            if os.path.exists(best_pt):",
    "                zipf.write(best_pt, arcname=model_name_zip)",
    "                logger.info(f'   + Añadido: {model_name_zip}')",
    "            ",
    "            # Add Vector Indices",
    "            indices_src = '/kaggle/working/indices'",
    "            if os.path.exists(indices_src):",
    "                for f in os.listdir(indices_src):",
    "                    if f.endswith('.pkl'):",
    "                        zipf.write(os.path.join(indices_src, f), arcname=f'indices/{f}')",
    "                        logger.info(f'   + Añadido: indices/{f}')",
    "            ",
    "            # Add Logs/Reports",
    "            if os.path.exists(report_path):",
    "                zipf.write(report_path, arcname='performance_report.json')",
    "        logger.info(f'✅ ZIP Creado Exitosamente: {zip_path}')",
    "        logger.info('⬇️  PUEDES DESCARGAR ESTE ARCHIVO DESDE LA PESTAÑA \"OUTPUT\" DE KAGGLE (Barra Lateral Derecha)')",
    "    except Exception as e:",
    "        logger.error(f'❌ Error creando el backup ZIP: {e}')",
]
